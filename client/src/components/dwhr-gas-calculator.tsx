...
import { Droplets, TrendingUp, Save } from "lucide-react";
...
// DWHR calculation input schema
const dwhreInputSchema = z.object({
...
type DwhreCalculationInputs = z.infer<typeof dwhreInputSchema>;

interface DwhreCalculationResults {
  annualHotWaterUse: number;
  annualEnergyRecovered: number;
  annualFuelSaved: number;
}

export default function DwhrGasCalculator() {
  ...
  const form = useForm<DwhreCalculationInputs>({
    resolver: zodResolver(dwhreInputSchema),
    defaultValues: {
      waterTempOut: 101,
      waterTempIn: 46.04,
  ...
  useEffect(() => {
    if (project?.dwhreData) {
      const data = project.dwhreData as any;
      form.reset({
  ...
  const saveToProject = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "DWHR calculation saved to project successfully.",
      });
    },
    onError: () => {
      toast({
  ...
  const calculateResults = (inputs: DwhreCalculationInputs): DwhreCalculationResults => {
    ...
    // Annual Fuel Saved = Annual Energy Recovered ÷ Gas Water Heater Efficiency
    const annualFuelSaved = annualEnergyRecovered / inputs.gasWaterHeaterEfficiency;
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="bg-blue-50 p-4 rounded-lg space-y-2 text-sm">
                        <div><strong>Measure Name:</strong> Drain Water Heat Recovery with a Natural Gas Water Heater</div>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <p className="text-xs text-gray-500">Source: Cadmus & Opinion Dynamics, 2013</p>
                              <FormMessage />
                            </FormItem>
                          )}
                        />
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <FormField
                          control={form.control}
                          name="gasWaterHeaterEfficiency"
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Gas Water Heater Efficiency</FormLabel>
                              <FormControl>
                                <Input 
                                  type="number" 
                                  step="0.01" 
